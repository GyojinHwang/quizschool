...
import { Game, Question, formattedQusetion } from "../../types";
...
const SubCreate = () => {
  ...
  const handleMessage = (event: MessageEvent) => {
    ...
    // const [formattedQuestions, setFormattedQuestions] = useState<formattedQusetion[]>([]);
    ...
    const game_no = String(gameCount);


    let router = useRouter();

    // const validation = async (event: FormEvent) => {
    //     event.preventDefault();
    //     try{
    //         const active = true;
    //         const res1 = await axios.post("/games/validator",
    //         {
    //             active, game_name, template_no, game_no,
    //         }
    //         )
    //         const res2 = await axios.post("formattedquestions/validator", {
    //             game_no,
    //             question1_title,
    //             question1_answer1,
    //             question1_answer2,
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            const response = await axios.post("/formattedquestions", {
                game_no,
                game_name,
                question1_title,
                question1_answer1,
            ...
            if (response.status == 200) { 
                const res = await axios.post("/games/create", { active, game_name, template_no, game_no })

                const { data: question } = await axios.post<Question>("/questions/", {
                    question1_no: "1",
